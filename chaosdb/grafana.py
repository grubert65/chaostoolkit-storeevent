...
def post_event(server: dict, payload: dict):
    ...
    headers = {
        ...
        'Authorization': 'Bearer '+ server['api_token'] if 'api_token' in server else None
    }

    data = json.dumps(payload)
    logger.debug(f"Sending annotation to grafana server {server['host']}:{server['port']}")
    logger.debug(f"Data:\n{data}")
    r = requests.post(
        f"{server['protocol']}://{server['host']}:{server['port']}{server['annotation_api_endpoint']}",
        auth=HTTPBasicAuth(server['username'], server['password']) if 'username' in server else None,
